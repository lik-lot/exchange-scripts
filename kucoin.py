"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (KUCOIN)
- Auto-collect all symbols with enableTrading=true from KuCoin REST API /api/v1/symbols
...
# ▼ Dependencies for automatic KUCOIN symbol collection
import requests
...
KUCOIN          = "KUCOIN"               # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_kucoin_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_kucoin_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_kucoin_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_kucoin_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_kucoin_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_kucoin_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "KUCOIN API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from KUCOIN) ─────────────────
KUCOIN_API = "https://api.kucoin.com"
SYMBOLS_ENDPOINT = "/api/v1/symbols"

def kucoin_to_tv_symbol(kucoin_symbol: str) -> str:
    """Convert KuCoin symbol format to TradingView format"""
    # Based on test results: BTC-USDT → BTCUSDT (remove dash)
    return kucoin_symbol.replace("-", "").upper()

def fetch_kucoin_all_spot_symbols() -> List[str]:
    """
    Return all symbols with enableTrading=true from KuCoin.
    """
    url = urljoin(KUCOIN_API, SYMBOLS_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != "200000":  # KuCoin success code
            logging.error(f"KuCoin API error: {data.get('msg', 'Unknown error')}")
            return []
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from KuCoin")
...
# Fetch symbols
logging.info("Fetching KuCoin symbols...")
symbols = fetch_kucoin_all_spot_symbols()
...
summary_file = "kucoin_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"KuCoin Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: KUCOIN\n")
