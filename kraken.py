"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (KRAKEN)
- Auto-collect all tradeable symbols from Kraken REST API /0/public/AssetPairs
...
# ▼ Dependencies for automatic KRAKEN symbol collection
import requests
...
KRAKEN          = "KRAKEN"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_kraken_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_kraken_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_kraken_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_kraken_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_kraken_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_kraken_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "KRAKEN API symbol" → "TradingView symbol"
MANUAL_MAP = {
    # Example mappings for Kraken's special naming (adjust if needed)
    # "XXBTZUSD": "BTCUSD",
}
...
# ─────────────────── 1. Load Symbols (Direct query from KRAKEN) ─────────────────
...
def fetch_kraken_all_spot_symbols() -> List[str]:
    ...
    - Filters out dark pool symbols (those ending with .d)
    """
    url = urljoin(KRAKEN_API, ASSET_PAIRS_ENDPOINT)
    ...
    try:
        ...
        logging.error(f"Failed to fetch Kraken symbols: {e}")
        return []

    syms = []
    for pair_name, pair_info in pairs_data.items():
        # Skip dark pool symbols
        if pair_name.endswith('.d'):
            continue

        # Use the altname if available, otherwise use the main pair name
        sym = pair_info.get("wsname", pair_name)
        if not sym:
            sym = pair_name
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from Kraken")
...
# Fetch symbols
logging.info("Fetching Kraken symbols...")
symbols = fetch_kraken_all_spot_symbols()
...
summary_file = "kraken_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Kraken Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: KRAKEN\n")
