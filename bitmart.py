"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BITMART)
- Auto-collect all symbols from BitMart REST API /spot/v1/symbols
...
# ▼ Dependencies for automatic BITMART symbol collection
import requests
...
BITMART        = "BITMART"              # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_bitmart_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_bitmart_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_bitmart_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_bitmart_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_bitmart_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_bitmart_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BITMART API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from BITMART) ─────────────────
...
def fetch_bitmart_all_spot_symbols() -> List[str]:
    ...
    """
    url = urljoin(BITMART_API, SYMBOLS_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != 1000:  # BitMart success code
            logging.error(f"BitMart API error: {data.get('message', 'Unknown error')}")
            return []

        symbols_data = data.get("data", {}).get("symbols", [])

    except Exception as e:
        logging.error(f"Failed to fetch BitMart symbols: {e}")
        return []
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from BitMart")
...
# Fetch symbols
logging.info("Fetching BitMart symbols...")
symbols = fetch_bitmart_all_spot_symbols()
...
summary_file = "bitmart_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"BitMart Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BITMART\n")
