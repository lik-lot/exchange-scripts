"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BTSE)
...
# ▼ Dependencies for automatic BTSE symbol collection
import requests
...
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_btse_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_btse_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_btse_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_btse_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_btse_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_btse_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BTSE API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
def fetch_btse_all_spot_symbols() -> List[str]:
    """
    Return all active symbols from BTSE.
    """
    url = urljoin(BTSE_API, MARKET_SUMMARY_ENDPOINT)
    
    headers = {
        "Accept": "application/json",
        "User-Agent": "btse-feed/1.0"
    }
    ...
    try:
        ...
        if not isinstance(data, list):
            logging.error(f"BTSE API unexpected response format")
            return []

        # BTSE returns array of market summary objects
        markets_data = data

    except Exception as e:
        logging.error(f"Failed to fetch BTSE symbols: {e}")
        return []
    ...
    major_symbols = ['BTCUSD', 'ETHUSD', 'LTCUSD', 'ADAUSD', 'SOLUSDT', 'BNBUSDT']
    ...
    logging.info(f"Found {len(unique_syms)} unique active symbols from BTSE")
...
# Fetch symbols
logging.info("Fetching BTSE symbols...")
symbols = fetch_btse_all_spot_symbols()
...
summary_file = "btse_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"BTSE Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BTSE\n")
