"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (HTX/Huobi)
- Auto-collect all 'online' symbols from HTX REST API /v1/common/symbols
...
# ▼ Dependencies for automatic HTX symbol collection
import requests
...
HTX             = "HUOBI"                # TradingView exchange code (uses HUOBI)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_htx_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_htx_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_htx_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_htx_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_htx_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_htx_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "HTX API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from HTX) ─────────────────
HTX_API = "https://api.huobi.pro"
SYMBOLS_ENDPOINT = "/v1/common/symbols"

def fetch_htx_all_spot_symbols() -> List[str]:
    """
    Return all 'online' symbols from HTX.
    """
    url = urljoin(HTX_API, SYMBOLS_ENDPOINT)
    ...
    try:
        ...
        if data.get("status") != "ok":
            logging.error(f"HTX API error: {data.get('err-msg', 'Unknown error')}")
            return []

        symbols_data = data.get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch HTX symbols: {e}")
        return []

    syms = []
    for s in symbols_data:
        # Check symbol state - should be 'online'
        ...
        if QUOTE_FILTER:
            quote_currency = s.get("quote-currency", "")
        ...
        # Convert to uppercase for TradingView compatibility (HTX symbols are lowercase)
        sym = sym.upper()
        syms.append(sym)
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from HTX")
...
# Fetch symbols
logging.info("Fetching HTX symbols...")
symbols = fetch_htx_all_spot_symbols()
...
summary_file = "htx_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"HTX Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: HTX (Huobi)\n")
