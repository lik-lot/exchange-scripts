"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (WHITEBIT)
- Auto-collect all spot symbols with tradesEnabled=true from WhiteBIT REST API /api/v4/public/markets
...
# ▼ Dependencies for automatic WHITEBIT symbol collection
import requests
...
WHITEBIT        = "WHITEBIT"             # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_whitebit_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_whitebit_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_whitebit_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_whitebit_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_whitebit_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_whitebit_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "WHITEBIT API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
def fetch_whitebit_all_spot_symbols() -> List[str]:
    ...
    """
    url = urljoin(WHITEBIT_API, MARKETS_ENDPOINT)
    ...
    try:
        ...
        if not isinstance(data, dict):
            logging.error(f"WhiteBIT API unexpected response format")
            return []

        # WhiteBIT returns a dict with symbol as key
        markets_data = data

    except Exception as e:
        logging.error(f"Failed to fetch WhiteBIT symbols: {e}")
        return []
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable spot symbols from WhiteBIT")
...
# Fetch symbols
logging.info("Fetching WhiteBIT symbols...")
symbols = fetch_whitebit_all_spot_symbols()
...
summary_file = "whitebit_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"WhiteBIT Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: WHITEBIT\n")
