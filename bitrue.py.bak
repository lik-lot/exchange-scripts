"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BITRUE)
- Auto-collect all symbols from Bitrue REST API /api/v1/ticker/24hr
...
# ▼ Dependencies for automatic BITRUE symbol collection
import requests
...
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_bitrue_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_bitrue_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_bitrue_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_bitrue_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_bitrue_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_bitrue_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BITRUE API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from BITRUE) ─────────────────
...
def fetch_bitrue_all_spot_symbols() -> List[str]:
    ...
    """
    url = urljoin(BITRUE_API, TICKER_ENDPOINT)
    ...
    try:
        ...
        if not isinstance(data, list):
            logging.error(f"Bitrue API unexpected response format")
            return []

        # Bitrue returns array of ticker objects
        tickers_data = data

    except Exception as e:
        logging.error(f"Failed to fetch Bitrue symbols: {e}")
        return []
...
# Fetch symbols
logging.info("Fetching Bitrue symbols...")
symbols = fetch_bitrue_all_spot_symbols()
...
summary_file = "bitrue_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Bitrue Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BITRUE\n")
