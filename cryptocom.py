"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (CRYPTO.COM)
- Auto-collect all active symbols from Crypto.com REST API /exchange/v1/public/get-instruments
...
# ▼ Dependencies for automatic CRYPTO.COM symbol collection
import requests
...
CRYPTOCOM       = "CRYPTOCOM"            # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_cryptocom_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_cryptocom_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_cryptocom_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_cryptocom_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_cryptocom_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_cryptocom_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "CRYPTO.COM API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from CRYPTO.COM) ─────────────────
...
def fetch_cryptocom_all_spot_symbols() -> List[str]:
    ...
    """
    url = urljoin(CRYPTOCOM_API, INSTRUMENTS_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != 0:  # Crypto.com success code
            logging.error(f"Crypto.com API error: {data.get('message', 'Unknown error')}")
            return []

        instruments_data = data.get("result", {}).get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch Crypto.com symbols: {e}")
        return []
...
# Fetch symbols
logging.info("Fetching Crypto.com symbols...")
symbols = fetch_cryptocom_all_spot_symbols()
...
summary_file = "cryptocom_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Crypto.com Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: CRYPTO.COM\n")
