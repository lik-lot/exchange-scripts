"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BICONOMY)
- Auto-collect all 'trading' symbols from Biconomy REST API /api/v1/exchangeInfo
...
# ▼ Dependencies for automatic BICONOMY symbol collection
import requests
...
BICONOMY        = "BICONOMY"             # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_biconomy_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_biconomy_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_biconomy_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_biconomy_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_biconomy_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_biconomy_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BICONOMY API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from BICONOMY) ─────────────────
BICONOMY_API = "https://www.biconomy.com"
EXCHANGE_INFO_ENDPOINT = "/api/v1/exchangeInfo"

def fetch_biconomy_all_spot_symbols() -> List[str]:
    """
    Return all 'trading' symbols from Biconomy.
    """
    url = urljoin(BICONOMY_API, EXCHANGE_INFO_ENDPOINT)
    
    headers = {
        "X-SITE-ID": "127",
        "User-Agent": "biconomy-feed/1.0",
    ...
    try:
        ...
        # Biconomy exchange info structure
        symbols_data = data.get("symbols", [])

    except Exception as e:
        logging.error(f"Failed to fetch Biconomy symbols: {e}")
        return []

    syms = []
    for s in symbols_data:
        # Check status - should be 'trading'
        ...
        if QUOTE_FILTER:
            ...
            if quote_asset not in QUOTE_FILTER:
                continue
    ...
    logging.info(f"Found {len(unique_syms)} unique trading symbols from Biconomy")
...
# Fetch symbols
logging.info("Fetching Biconomy symbols...")
symbols = fetch_biconomy_all_spot_symbols()
...
summary_file = "biconomy_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Biconomy Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BICONOMY\n")
