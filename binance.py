"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BINANCE)
- Auto-collect all 'SPOT + TRADING' symbols from Binance REST /api/v3/exchangeInfo
...
# ▼ Dependencies for automatic BINANCE symbol collection
import requests
...
BINANCE         = "BINANCE"              # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_1d"},
]

# (Optional) Exclusion rules
EXCLUDE_LEVERAGED = True                 # Exclude leveraged/derivative tokens
EXCLUDE_TAGS      = ("UP", "DOWN", "BULL", "BEAR")   # Exclude if symbol contains these
EXCLUDE_SUFFIXES  = ("3L", "3S", "5L", "5S")         # Exclude suffixes

# (Optional) Manual mapping: "BINANCE symbol" → "TradingView symbol"
MANUAL_MAP = {
    # "WBTCBTC": "WBTCBTC",   # Example (use if needed)
}
...
# ─────────────────── 1. Load Symbols (Direct query from BINANCE) ─────────────────
BINANCE_API = "https://api.binance.com"
EXCHANGE_INFO = "/api/v3/exchangeInfo"

def fetch_binance_all_spot_symbols() -> List[str]:
    """
    Return all tradeable (TRADING) symbols from BINANCE spot.
    - No quoteAsset restrictions (includes all markets: USDT, BTC, ETH, BUSD, etc.)
    - Leveraged/derivative tokens excluded by default (when EXCLUDE_LEVERAGED=True)
    """
    url = urljoin(BINANCE_API, EXCHANGE_INFO)
    ...
    for s in data.get("symbols", []):
        ...
        if EXCLUDE_LEVERAGED:
            # Simple heuristic: exclude leveraged/derivative token patterns
            if any(tag in sym for tag in EXCLUDE_TAGS):
                continue
...
symbols = fetch_binance_all_spot_symbols()
...
summary_file = "collection_summary.txt"
