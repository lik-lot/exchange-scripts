"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (HASHKEY GLOBAL)
- Auto-collect all SPOT symbols from HashKey Global REST API /quote/v1/ticker/24hr?instType=SPOT
...
# ▼ Dependencies for automatic HASHKEY GLOBAL symbol collection
import requests
...
HASHKEYGLOBAL   = "HASHKEYGLOBAL"        # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_hashkeyglobal_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_hashkeyglobal_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_hashkeyglobal_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_hashkeyglobal_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_hashkeyglobal_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_hashkeyglobal_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "HASHKEY GLOBAL API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from HASHKEY GLOBAL) ─────────────────
HASHKEYGLOBAL_API = "https://api-glb.hashkey.com"
TICKER_ENDPOINT = "/quote/v1/ticker/24hr?instType=SPOT"

def fetch_hashkeyglobal_all_spot_symbols() -> List[str]:
    """
    Return all SPOT symbols from HashKey Global.
    """
    url = urljoin(HASHKEYGLOBAL_API, TICKER_ENDPOINT)
    ...
    try:
        ...
        if not isinstance(data, list):
            logging.error(f"HashKey Global API unexpected response format")
            return []

        # HashKey Global returns array of ticker objects
        tickers_data = data

    except Exception as e:
        logging.error(f"Failed to fetch HashKey Global symbols: {e}")
        return []

    syms = []
    for ticker in tickers_data:
        # Get the symbol name (HashKey Global uses 's' field for symbol)
        sym = ticker.get("s", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique SPOT symbols from HashKey Global")
...
# Fetch symbols
logging.info("Fetching HashKey Global SPOT symbols...")
symbols = fetch_hashkeyglobal_all_spot_symbols()
...
summary_file = "hashkeyglobal_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"HashKey Global Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: HASHKEY GLOBAL\n")
