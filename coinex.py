"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (COINEX)
- Auto-collect all symbols from CoinEx REST API /v2/spot/ticker
...
# ▼ Dependencies for automatic COINEX symbol collection
import requests
...
COINEX          = "COINEX"               # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_coinex_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_coinex_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_coinex_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_coinex_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_coinex_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_coinex_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "COINEX API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from COINEX) ─────────────────
COINEX_API = "https://api.coinex.com"
TICKER_ENDPOINT = "/v2/spot/ticker"

def fetch_coinex_all_spot_symbols() -> List[str]:
    """
    Return all symbols from CoinEx.
    """
    url = urljoin(COINEX_API, TICKER_ENDPOINT)
    
    headers = {
        "Accept": "application/json",
        "User-Agent": "coinex-feed/1.0"
    }
    ...
    try:
        ...
        if data.get("code") != 0:  # CoinEx success code
            logging.error(f"CoinEx API error: {data.get('message', 'Unknown error')}")
            return []

        tickers_data = data.get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch CoinEx symbols: {e}")
        return []

    syms = []
    for ticker in tickers_data:
        # Get the symbol name (CoinEx uses 'market' field)
        sym = ticker.get("market", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from CoinEx")
...
# Fetch symbols
logging.info("Fetching CoinEx symbols...")
symbols = fetch_coinex_all_spot_symbols()
...
summary_file = "coinex_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"CoinEx Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: COINEX\n")
