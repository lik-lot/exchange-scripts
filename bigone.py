"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BIGONE)
- Auto-collect all symbols from BigONE REST API /api/v3/asset_pairs/tickers
- Extracts multiple timeframes: 1m, 5m, 15m, 1h, 4h, 1d
- Rate-limit (4 calls/sec), random jitter, exponential back-off retry included
- Includes fallback to v2 API if v3 fails
"""
...
# ▼ Dependencies for automatic BIGONE symbol collection
import requests
...
BIGONE          = "BIGONE"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_bigone_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_bigone_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_bigone_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_bigone_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_bigone_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_bigone_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BIGONE API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from BIGONE) ─────────────────
BIGONE_API = "https://big.one"
TICKERS_ENDPOINT_V3 = "/api/v3/asset_pairs/tickers"
TICKERS_ENDPOINT_V2 = "/api/v2/tickers"

def fetch_bigone_all_spot_symbols() -> List[str]:
    """
    Return all symbols from BigONE. Try v3 API first, fallback to v2.
    """
    headers = {
        "Accept": "application/json",
        "User-Agent": "bigone-feed/1.0"
    }

    # Try v3 API first
    try:
        url = urljoin(BIGONE_API, TICKERS_ENDPOINT_V3)
        r = requests.get(url, headers=headers, timeout=30)
        r.raise_for_status()
        data = r.json()

        if data.get("code") == 0:  # BigONE success code
            tickers_data = data.get("data", [])
            api_version = "v3"
        else:
            raise Exception(f"BigONE v3 API error: {data}")

    except Exception as e:
        logging.warning(f"BigONE v3 API failed: {e}, trying v2...")
        
        # Fallback to v2 API
        try:
            url = urljoin(BIGONE_API, TICKERS_ENDPOINT_V2)
            r = requests.get(url, headers=headers, timeout=30)
            r.raise_for_status()
            data = r.json()
            ...
            if data.get("code") == 0:  # BigONE success code
                ...
                raise Exception(f"BigONE v2 API error: {data}")

        except Exception as e2:
            logging.error(f"Failed to fetch BigONE symbols from both v3 and v2 APIs: {e2}")
            return []

    syms = []
    for ticker in tickers_data:
        # Get the symbol name - different field names for different API versions
        if api_version == "v3":
            sym = ticker.get("market_id", "") or ticker.get("asset_pair_name", "")
        else:  # v2
            sym = ticker.get("market", "") or ticker.get("symbol", "")
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from BigONE (using {api_version} API)")
...
# Fetch symbols
logging.info("Fetching BigONE symbols...")
symbols = fetch_bigone_all_spot_symbols()
...
summary_file = "bigone_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"BigONE Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BIGONE\n")
