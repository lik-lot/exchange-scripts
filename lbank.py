"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (LBANK)
- Auto-collect all symbols from LBank REST API /v2/currencyPairs.do
...
# ▼ Dependencies for automatic LBANK symbol collection
import requests
...
LBANK           = "LBANK"                # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_lbank_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_lbank_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_lbank_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_lbank_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_lbank_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_lbank_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "LBANK API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from LBANK) ─────────────────
LBANK_API = "https://api.lbkex.com"
CURRENCY_PAIRS_ENDPOINT = "/v2/currencyPairs.do"

def fetch_lbank_all_spot_symbols() -> List[str]:
    """
    Return all symbols from LBank.
    """
    url = urljoin(LBANK_API, CURRENCY_PAIRS_ENDPOINT)
    ...
    try:
        ...
        if not data.get("result"):
            logging.error(f"LBank API error: {data}")
            return []

        symbols_data = data.get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch LBank symbols: {e}")
        return []

    syms = []
    for symbol_str in symbols_data:
        # LBank returns array of symbol strings like "btc_usdt"
        ...
        # Convert to uppercase for TradingView compatibility (LBank symbols are lowercase)
        sym = sym.upper()
        syms.append(sym)
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from LBank")
...
# Fetch symbols
logging.info("Fetching LBank symbols...")
symbols = fetch_lbank_all_spot_symbols()
...
summary_file = "lbank_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"LBank Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: LBANK\n")
