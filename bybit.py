"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BYBIT)
...
# ▼ Dependencies for automatic BYBIT symbol collection
import requests
...
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_bybit_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_bybit_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_bybit_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_bybit_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_bybit_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_bybit_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BYBIT API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
def fetch_bybit_all_spot_symbols() -> List[str]:
    """
    Return all active SPOT symbols from BYBIT.
    """
    url = urljoin(BYBIT_API, INSTRUMENTS_ENDPOINT)
    ...
    try:
        ...
        if data.get("retCode") != 0:  # Bybit success code
            logging.error(f"Bybit API error: {data.get('retMsg', 'Unknown error')}")
            return []

        instruments_data = data.get("result", {}).get("list", [])

    except Exception as e:
        logging.error(f"Failed to fetch Bybit symbols: {e}")
        return []

    syms = []
    for s in instruments_data:
        # Get the symbol name (Bybit uses 'symbol' field)
        sym = s.get("symbol", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique SPOT symbols from Bybit")
...
symbols = fetch_bybit_all_spot_symbols()
...
summary_file = "bybit_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Bybit Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BYBIT\n")
