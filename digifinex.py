"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (DIGIFINEX)
- Auto-collect all symbols from DigiFinex REST API /v3/ticker
...
# ▼ Dependencies for automatic DIGIFINEX symbol collection
import requests
...
DIGIFINEX       = "DIGIFINEX"            # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_digifinex_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_digifinex_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_digifinex_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_digifinex_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_digifinex_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_digifinex_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "DIGIFINEX API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from DIGIFINEX) ─────────────────
DIGIFINEX_API = "https://openapi.digifinex.com"
TICKER_ENDPOINT = "/v3/ticker"

def fetch_digifinex_all_spot_symbols() -> List[str]:
    """
    Return all symbols from DigiFinex.
    """
    url = urljoin(DIGIFINEX_API, TICKER_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != 0:  # DigiFinex success code
            logging.error(f"DigiFinex API error: {data.get('message', 'Unknown error')}")
            return []

        ticker_data = data.get("ticker", [])

    except Exception as e:
        logging.error(f"Failed to fetch DigiFinex symbols: {e}")
        return []
    ...
    for ticker in ticker_data:
        ...
        # Convert to uppercase for TradingView compatibility (DigiFinex symbols are lowercase)
        sym = sym.upper()
        syms.append(sym)
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from DigiFinex")
...
# Fetch symbols
logging.info("Fetching DigiFinex symbols...")
symbols = fetch_digifinex_all_spot_symbols()
...
summary_file = "digifinex_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"DigiFinex Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: DIGIFINEX\n")
