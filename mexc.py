"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (MEXC)
- Auto-collect all TRADING symbols from MEXC REST API /api/v3/exchangeInfo
...
# ▼ Dependencies for automatic MEXC symbol collection
import requests
...
MEXC            = "MEXC"                 # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_mexc_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_mexc_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_mexc_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_mexc_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_mexc_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_mexc_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "MEXC API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from MEXC) ─────────────────
MEXC_API = "https://api.mexc.com"
EXCHANGE_INFO_ENDPOINT = "/api/v3/exchangeInfo"

def fetch_mexc_all_spot_symbols() -> List[str]:
    """
    Return all TRADING symbols from MEXC with status = '1' or 'ENABLED' or 'ONLINE'
    and isSpotTradingAllowed = True.
    """
    url = urljoin(MEXC_API, EXCHANGE_INFO_ENDPOINT)
    ...
    try:
        ...
        logging.error(f"Failed to fetch MEXC symbols: {e}")
        return []

    syms = []
    for s in symbols_data:
        # Check symbol status - should be active
        status = s.get("status", "")
        if status not in ["1", "ENABLED", "ONLINE"]:
            continue
        ...
        # Check permissions - must include SPOT
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from MEXC")
...
# Fetch symbols
logging.info("Fetching MEXC symbols...")
symbols = fetch_mexc_all_spot_symbols()
...
summary_file = "mexc_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"MEXC Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: MEXC\n")
