"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (TOOBIT)
- Auto-collect all symbols from Toobit REST API /quote/v1/ticker/24hr
...
# ▼ Dependencies for automatic TOOBIT symbol collection
import requests
...
TOOBIT          = "TOOBIT"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_toobit_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_toobit_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_toobit_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_toobit_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_toobit_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_toobit_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "TOOBIT API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from TOOBIT) ─────────────────
TOOBIT_API = "https://api.toobit.com"
TICKER_ENDPOINT = "/quote/v1/ticker/24hr"

def fetch_toobit_all_spot_symbols() -> List[str]:
    """
    Return all symbols from Toobit.
    """
    url = urljoin(TOOBIT_API, TICKER_ENDPOINT)
    ...
    try:
        ...
        if not isinstance(data, list):
            logging.error(f"Toobit API unexpected response format")
            return []

        # Toobit returns array of ticker objects
        symbols_data = data

    except Exception as e:
        logging.error(f"Failed to fetch Toobit symbols: {e}")
        return []

    syms = []
    for ticker in symbols_data:
        # Get the symbol name (Toobit uses 's' field for symbol)
        sym = ticker.get("s", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from Toobit")
...
# Fetch symbols
logging.info("Fetching Toobit symbols...")
symbols = fetch_toobit_all_spot_symbols()
...
summary_file = "toobit_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Toobit Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: TOOBIT\n")
