"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (DEEPCOIN)
- Auto-collect all 'live' SPOT symbols from Deepcoin REST API /deepcoin/market/instruments?instType=SPOT
...
# ▼ Dependencies for automatic DEEPCOIN symbol collection
import requests
...
DEEPCOIN        = "DEEPCOIN"             # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_deepcoin_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_deepcoin_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_deepcoin_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_deepcoin_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_deepcoin_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_deepcoin_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "DEEPCOIN API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from DEEPCOIN) ─────────────────
DEEPCOIN_API = "https://api.deepcoin.com"
INSTRUMENTS_ENDPOINT = "/deepcoin/market/instruments?instType=SPOT"

def fetch_deepcoin_all_spot_symbols() -> List[str]:
    """
    Return all 'live' SPOT symbols from Deepcoin.
    """
    url = urljoin(DEEPCOIN_API, INSTRUMENTS_ENDPOINT)
    
    headers = {
        "User-Agent": "deepcoin-feed/1.0",
    ...
    try:
        ...
        if data.get("code") != "0":  # Deepcoin success code
            logging.error(f"Deepcoin API error: {data.get('msg', 'Unknown error')}")
            return []

        instruments_data = data.get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch Deepcoin symbols: {e}")
        return []

    syms = []
    for s in instruments_data:
        # Check state - should be 'live'
        if s.get("state") != "live":
            continue

        # Get the symbol name (Deepcoin uses 'instId' field)
        sym = s.get("instId", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique live SPOT symbols from Deepcoin")
...
# Fetch symbols
logging.info("Fetching Deepcoin SPOT symbols...")
symbols = fetch_deepcoin_all_spot_symbols()
...
summary_file = "deepcoin_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Deepcoin Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: DEEPCOIN\n")
