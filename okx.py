"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (OKX)
...
# ▼ Dependencies for automatic OKX symbol collection
import requests
...
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_okx_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_okx_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_okx_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_okx_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_okx_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_okx_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "OKX API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
def fetch_okx_all_spot_symbols() -> List[str]:
    """
    Return all active SPOT symbols from OKX.
    """
    url = urljoin(OKX_API, INSTRUMENTS_ENDPOINT)
    ...
    for s in instruments_data:
        # Get the symbol name (OKX uses 'instId' field)
        sym = s.get("instId", "")
        if not sym:
            continue
    ...
    logging.info(f"Found {len(unique_syms)} unique SPOT symbols from OKX")
...
# Fetch symbols
logging.info("Fetching OKX SPOT symbols...")
symbols = fetch_okx_all_spot_symbols()
...
summary_file = "okx_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"OKX Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: OKX\n")
