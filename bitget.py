"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (BITGET)
- Auto-collect all 'SPOT' symbols from Bitget REST API /api/v2/spot/public/symbols
...
# ▼ Dependencies for automatic BITGET symbol collection
import requests
...
BITGET          = "BITGET"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_bitget_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_bitget_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_bitget_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_bitget_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_bitget_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_bitget_1d"},
]

# (Optional) Exclusion rules
EXCLUDE_LEVERAGED = True                 # Exclude leveraged/derivative tokens
EXCLUDE_TAGS      = ("UP", "DOWN", "BULL", "BEAR", "2L", "2S", "3L", "3S", "5L", "5S")   # Exclude if symbol contains these
EXCLUDE_SUFFIXES  = ("3L", "3S", "5L", "5S", "2L", "2S")         # Exclude suffixes

# (Optional) Include only specific quote currencies (empty list = all)
# QUOTE_FILTER = ["USDT", "BTC", "ETH"]  # Uncomment to filter
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "BITGET API symbol" → "TradingView symbol"
MANUAL_MAP = {
    # Example mappings (adjust if needed):
    # "BTCUSDT_SPBL": "BTCUSDT",   # Bitget API might have _SPBL suffix
}
...
# ─────────────────── 1. Load Symbols (Direct query from BITGET) ─────────────────
BITGET_API = "https://api.bitget.com"
SPOT_SYMBOLS_ENDPOINT = "/api/v2/spot/public/symbols"

def fetch_bitget_all_spot_symbols() -> List[str]:
    """
    Return all tradeable ('online') symbols from BITGET spot.
    - Filters based on status='online'
    - Optionally filters by quote currency
    - Excludes leveraged tokens based on configuration
    """
    url = urljoin(BITGET_API, SPOT_SYMBOLS_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != "00000":  # Bitget success code
            logging.error(f"Bitget API error: {data.get('msg', 'Unknown error')}")
            return []

        symbols_data = data.get("data", [])

    except Exception as e:
        logging.error(f"Failed to fetch Bitget symbols: {e}")
        return []

    syms = []
    for s in symbols_data:
        # Check if symbol is online (tradeable)
        if s.get("status") != "online":
            continue

        # Get the symbol name (Bitget uses 'symbol' field)
        sym = s.get("symbol", "")
        if not sym:
            continue

        # Remove any suffixes like _SPBL if present
        if "_SPBL" in sym:
        ...
        # Exclude leveraged/derivative tokens
        if EXCLUDE_LEVERAGED:
            sym_upper = sym.upper()
            if any(tag in sym_upper for tag in EXCLUDE_TAGS):
                logging.debug(f"Excluding leveraged token: {sym}")
                continue
            if any(sym_upper.endswith(suf) for suf in EXCLUDE_SUFFIXES):
                logging.debug(f"Excluding token with suffix: {sym}")
                continue
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from Bitget")
...
# Fetch symbols
logging.info("Fetching Bitget spot symbols...")
symbols = fetch_bitget_all_spot_symbols()
...
summary_file = "bitget_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Bitget Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: BITGET\n")
