"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (COINW)
- Auto-collect all symbols from CoinW REST API /api/v1/public?command=returnTicker
...
# ▼ Dependencies for automatic COINW symbol collection
import requests
...
COINW           = "COINW"                # TradingView exchange code (VERIFIED WORKING)

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_coinw_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_coinw_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_coinw_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_coinw_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_coinw_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_coinw_1d"},
]

# Mirror URLs for CoinW
MIRROR_URLS = [
    "https://api.coinw.com",
    "https://api.coinw.pw"
]
...
# (Optional) Manual mapping: "COINW API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
def fetch_coinw_all_spot_symbols() -> List[str]:
    ...
    for base_url in MIRROR_URLS:
        ...
        try:
            ...
            if "data" not in data:
                logging.error(f"CoinW API unexpected response format from {base_url}")
                continue
            ...
            logging.warning(f"Failed to fetch CoinW symbols from {base_url}: {e}")
            if base_url == MIRROR_URLS[-1]:  # Last URL
                logging.error("Failed to fetch from all CoinW mirror URLs")
                return []
            continue

    syms = []
    for symbol_key, ticker_info in ticker_data.items():
...
# Fetch symbols
logging.info("Fetching CoinW symbols...")
symbols = fetch_coinw_all_spot_symbols()
...
summary_file = "coinw_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"CoinW Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: COINW\n")
