"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (COINBASE)
- Auto-collect all active symbols from Coinbase Pro REST API /products
...
# ▼ Dependencies for automatic COINBASE symbol collection
import requests
...
COINBASE        = "COINBASE"             # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_coinbase_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_coinbase_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_coinbase_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_coinbase_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_coinbase_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_coinbase_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "COINBASE API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from COINBASE) ─────────────────
...
def fetch_coinbase_all_spot_symbols() -> List[str]:
    ...
    - Filters based on status='online' and trading_disabled=False
    - Optionally filters by quote currency
    """
    url = urljoin(COINBASE_API, PRODUCTS_ENDPOINT)
    ...
    try:
        ...
        logging.error(f"Failed to fetch Coinbase symbols: {e}")
        return []

    syms = []
    for s in data:
        # Check if symbol is active
        if s.get("status") != "online" or s.get("trading_disabled", True):
            continue

        # Get the symbol name (Coinbase uses 'id' field)
        sym = s.get("id", "")
        if not sym:
            continue
        ...
        if QUOTE_FILTER:
            quote_coin = s.get("quote_currency", "")
    ...
    logging.info(f"Found {len(unique_syms)} unique tradeable symbols from Coinbase")
...
# Fetch symbols
logging.info("Fetching Coinbase symbols...")
symbols = fetch_coinbase_all_spot_symbols()
...
summary_file = "coinbase_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Coinbase Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: COINBASE\n")
