"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (GEMINI)
- Auto-collect all symbols from Gemini REST API /v1/symbols
...
# ▼ Dependencies for automatic GEMINI symbol collection
import requests
...
GEMINI          = "GEMINI"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_gemini_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_gemini_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_gemini_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_gemini_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_gemini_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_gemini_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "GEMINI API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from GEMINI) ─────────────────
GEMINI_API = "https://api.gemini.com"
SYMBOLS_ENDPOINT = "/v1/symbols"

def fetch_gemini_all_spot_symbols() -> List[str]:
    """
    Return all symbols from Gemini.
    """
    url = urljoin(GEMINI_API, SYMBOLS_ENDPOINT)
    ...
    try:
        ...
        # Gemini returns array of symbol strings directly
        if not isinstance(data, list):
            logging.error(f"Gemini API unexpected response format")
            return []

        symbols_data = data

    except Exception as e:
        logging.error(f"Failed to fetch Gemini symbols: {e}")
        return []

    syms = []
    for symbol_str in symbols_data:
        # Gemini returns array of symbol strings like "btcusd"
        ...
        # Convert to uppercase for TradingView compatibility (Gemini symbols are lowercase)
        sym = sym.upper()
        syms.append(sym)
    ...
    logging.info(f"Found {len(unique_syms)} unique symbols from Gemini")
...
# Fetch symbols with rate limit delay
time.sleep(0.1)  # Gemini has low rate limits
logging.info("Fetching Gemini symbols...")
symbols = fetch_gemini_all_spot_symbols()
...
def wait_for_slot():
    ...
    time.sleep(0.1)  # Extra delay for Gemini's low rate limits
...
summary_file = "gemini_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Gemini Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: GEMINI\n")
