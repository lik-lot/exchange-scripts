"""
TradingView tvDatafeed v2.x  Multi-Timeframe Data Collection (PIONEX)
- Auto-collect all SPOT symbols from Pionex REST API /api/v1/market/tickers?type=SPOT
...
# ▼ Dependencies for automatic PIONEX symbol collection
import requests
...
PIONEX          = "PIONEX"               # TradingView exchange code

# Timeframes to collect with their respective intervals and save directories
TIMEFRAMES = [
    {"interval": Interval.in_1_minute,  "suffix": "1m",  "dir": "data_pionex_1m"},
    {"interval": Interval.in_5_minute,  "suffix": "5m",  "dir": "data_pionex_5m"},
    {"interval": Interval.in_15_minute, "suffix": "15m", "dir": "data_pionex_15m"},
    {"interval": Interval.in_1_hour,    "suffix": "1h",  "dir": "data_pionex_1h"},
    {"interval": Interval.in_4_hour,    "suffix": "4h",  "dir": "data_pionex_4h"},
    {"interval": Interval.in_daily,     "suffix": "1d",  "dir": "data_pionex_1d"},
]

# (Optional) Include only specific quote currencies (empty list = all)
QUOTE_FILTER = []                        # Empty = include all quote currencies

# (Optional) Manual mapping: "PIONEX API symbol" → "TradingView symbol"
MANUAL_MAP = {
...
# ─────────────────── 1. Load Symbols (Direct query from PIONEX) ─────────────────
PIONEX_API = "https://api.pionex.com"
TICKERS_ENDPOINT = "/api/v1/market/tickers?type=SPOT"

def fetch_pionex_all_spot_symbols() -> List[str]:
    """
    Return all SPOT symbols from Pionex.
    """
    url = urljoin(PIONEX_API, TICKERS_ENDPOINT)
    ...
    try:
        ...
        if data.get("code") != 0:  # Pionex success code
            logging.error(f"Pionex API error: {data.get('message', 'Unknown error')}")
            return []

        tickers_data = data.get("data", {}).get("tickers", [])

    except Exception as e:
        logging.error(f"Failed to fetch Pionex symbols: {e}")
        return []
    ...
    logging.info(f"Found {len(unique_syms)} unique SPOT symbols from Pionex")
...
# Fetch symbols
logging.info("Fetching Pionex SPOT symbols...")
symbols = fetch_pionex_all_spot_symbols()
...
summary_file = "pionex_collection_summary.txt"
with open(summary_file, "w") as f:
    f.write(f"Pionex Collection Summary - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    f.write("="*70 + "\n\n")
    f.write(f"Exchange: PIONEX\n")
